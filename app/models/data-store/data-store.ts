  ...
  .actions(self => ({
    ...
    refreshBalance: flow(function* () {
      ...
      if (result.kind === "ok") {
        ...
        self.user.setActiveBalance(data.activeBalance)
        self.user.setDeactivatedBalance(data.deactivatedBalance)
        self.user.setInput(data.input)
        self.user.setOutput(data.output)
      }
    }),
    listCoupons: flow(function* (payload) {
      const result = yield self.environment.api.post("/user/coupon/list", payload, self.user.token)
      self.transaction.setCode(result.payload.code)
      self.transaction.setStatus(result.kind)
      if (result.kind === "ok") {
        ...
        for (let coupon of coupons) {
          let item = CouponModel.create({
            ...
            expiry: moment(coupon.expiryDate).format("MM-DD-YYYY"),
            checkmark: "blank"
          })
          self.addCoupon(item)
        }
