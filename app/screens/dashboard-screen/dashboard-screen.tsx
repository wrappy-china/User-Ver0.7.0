...
export class DashboardScreen extends React.Component<DashboardScreenProps, {}> {
  ...
  //doSummary = () => this.props.navigation.navigate("summaryScreen", { name: "Bobby Ishimizu", id: "bobby@wrapious", type: "CONSUMER" , caption: "Transfer" })
  doSummary = () => this.props.navigation.navigate("summaryScreen", { name: "Sushi Restaurant", id: "sushi@wrapious", type: "STORE", caption: "Redeem" })
  doLogin = () => {
  ...
  doRefresh = () => {
    ...
    this.props.dataStore.refreshBalance().then(() => {
      this.props.dataStore.listCoupons({ filter: "ACTIVE" }).then(() => {
          ...
          <Grid>
            <Row style={{ height: 70 }}>
              ...
              <Col style={{ background: color.palette.white, alignSelf: 'center' }}>
                ...
                <Row style={{ height: 20 }}>
                  <Col style={{ width: 150 }}>
                    <Text style={{ fontSize: 13, color: color.palette.black }}>{item.id}</Text>
                  </Col>
                  <Col style={{ width: 80 }}>
                    <Text style={{ fontSize: 13, color: color.palette.black }}>{item.value}</Text>
                  </Col>
                </Row>
                <Row style={{ height: 2 }} />
                <Row style={{ height: 14 }}>
                ...
                <Row style={{ height: 22 }}>
                  <Col style={{ width: 150 }}>
                    <Text style={{ fontSize: 13, color: color.palette.black }}>{item.expiry}</Text>
                  </Col>
                  <Col style={{ width: 80 }}>
                    <Text style={{ fontSize: 13, color: color.palette.black }}>{item.status}</Text>
                  </Col>
                </Row>
                <Row />
              </Col>
        ...
        <LinearGradient useAngle={true} angle={135} colors={color.palette.gradientGreen} style={{ height: 250, width: global.SCREEN_WIDTH }}>
          <SafeAreaView style={{ alignItems: 'flex-start' }}>
            <Grid style={{ padding: 10, width: global.SCREEN_WIDTH }}>
              ...
              <Row style={{ height: 20 }} />
              <Row style={{ height: 30 }}>
                <Col style={{ alignSelf: 'center', alignItems: 'center' }}><Text style={{ fontFamily: 'Montserrat', fontSize: 28, color: color.palette.white }}>{this.props.dataStore.user.activeBalance}</Text></Col>
              </Row>
              <Row style={{ height: 20 }}>
              ...
              <Row style={{ height: 18 }}>
                ...
                <Col style={{ alignSelf: 'center', alignItems: 'center' }}>
                  <Text style={{ fontSize: 20, color: color.palette.white }}>{this.props.dataStore.user.deactivatedBalance}</Text>
                  <Row style={{ height: 5 }} />
                ...
                <Col style={{ alignSelf: 'center', alignItems: 'center' }}>
                  ...
                  <Row style={{ height: 5 }} />
                  <Text style={{ fontSize: 12, color: color.palette.mutedGray }}>Redeemed</Text>
                </Col>
              </Row>
            </Grid>
          </SafeAreaView>
